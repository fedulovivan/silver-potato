...
def gpio_setup():
  ...
  for pin_num in BUTTON_PINS:
    ...
    log.info('GPIO pin {0} set as IN and pulled up'.format(pin_num))
    GPIO.add_event_detect(pin_num, GPIO.FALLING, callback=btn_handler, bouncetime=300)
    log.info('GPIO pin {0} registered callback on FALLING'.format(pin_num))

def btn_handler(pin_num):
  log.info('GPIO pin {0} button pressed'.format(pin_num))
  stat = client.status()
  state = stat.get("state")
  log.info('mpd current state={0}'.format(state))
  if pin_num == BTN_1:
    ...
    log.warn('do power off')
  else:
    log.error('unexpected pin {0} in btn_handler'.format(pin_num))
